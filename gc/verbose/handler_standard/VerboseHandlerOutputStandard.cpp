...
MM_VerboseHandlerOutputStandard::handleScavengeEndNoLock(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
	uint64_t duration = 0;
	bool deltaTimeSuccess = getTimeDeltaInMicroSeconds(&duration, event->startTime, event->endTime);
...
MM_VerboseHandlerOutputStandard::handleConcurrentEndInternal(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	if (OMR_GC_CYCLE_TYPE_GLOBAL == stats->_concurrentCycleType) {
		...
		scavengeEndEvent.startTime = stats->_startTime;
		scavengeEndEvent.endTime = stats->_endTime;
