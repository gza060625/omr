...
MM_Dispatcher *
MM_Dispatcher::newInstance(MM_EnvironmentBase *env, omrsig_handler_fn handler, void* handler_arg, uintptr_t defaultOSStackSize)
{
	MM_Dispatcher *dispatcher;
	
	dispatcher = (MM_Dispatcher *)env->getForge()->allocate(sizeof(MM_Dispatcher), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (dispatcher) {
		new(dispatcher) MM_Dispatcher(env, handler, handler_arg, defaultOSStackSize);
		if(!dispatcher->initialize(env)) {
			dispatcher->kill(env);
			return NULL;
		}
...
MM_Dispatcher::initialize(MM_EnvironmentBase *env)
{
	...
	if(omrthread_monitor_init_with_name(&_slaveThreadMutex, 0, "MM_Dispatcher::slaveThread")
	|| omrthread_monitor_init_with_name(&_dispatcherMonitor, 0, "MM_Dispatcher::dispatcherControl")
	|| omrthread_monitor_init_with_name(&_synchronizeMutex, 0, "MM_Dispatcher::synchronize")) {
 ...
 */
uintptr_t
MM_Dispatcher::recomputeActiveThreadCountForTask(MM_EnvironmentBase *env, MM_Task *task, uintptr_t threadCount)
{
 ...
 */
void
MM_Dispatcher::slaveEntryPoint(MM_EnvironmentBase *env) 
{
	uintptr_t slaveID = env->getSlaveID();
...
void
MM_Dispatcher::masterEntryPoint(MM_EnvironmentBase *env)
{
 ...
 */
uintptr_t 
MM_Dispatcher::getThreadPriority()
{
 ...
 */
void
MM_Dispatcher::wakeUpThreads(uintptr_t count)
{
	omrthread_monitor_notify_all(_slaveThreadMutex);	
}
 ...
 */
void
MM_Dispatcher::setThreadInitializationComplete(MM_EnvironmentBase *env)
{
	uintptr_t slaveID = env->getSlaveID();
...
uintptr_t 
MM_Dispatcher::adjustThreadCount(uintptr_t maxThreadCount)
{
...
void
MM_Dispatcher::reinitAfterFork(MM_EnvironmentBase *env, uintptr_t newThreadCount)
{
	/* Set the slave thread mode to dying */
	for(uintptr_t index=0; index < _threadCountMaximum; index++) {
		_statusTable[index] = slave_status_dying;
